...
if [ -e /mnt/boot/usr/sbin/lvm.static ]
	...
	tell_kmsg "Herrie [initramfs] Activating LVM..."
    export LVM_SYSTEM_DIR=/mnt/boot/etc/lvm 
    tell_kmsg "Herrie LVM_SYSTEM_DIR: $LVM_SYSTEM_DIR"
    sleep 3
    /mnt/boot/usr/sbin/lvm.static vgchange -ay
    #sleep 3
    #/mnt/boot/usr/sbin/lvm.static vgchange -ay 2>&1 > /dev/kmsg
    #sleep 3
...
do
   tell_kmsg "Herrie partition in dev/store: $g"
done
...
process_bind_mounts() {
    tell_kmsg "Herrie we are using process_bind_mounts from S01-mount-boot.sh"
    tell_kmsg "userdata is vfat: override process_bind_mounts"
    tell_kmsg "Herrie rootmnt: ${rootmnt}"
    datadir=${rootmnt}/userdata/$distro_name-data
    tell_kmsg "Herrie Preparing datadir: $datadir"
